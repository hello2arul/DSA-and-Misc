...
public class MaximumNumberOfKDivisibleComponents {
    public int maxKDivisibleComponents(int n, int[][] edges, int[] values, int k) {
        ...
        dfs(0, adj, res, values, k, new HashSet<>());
        return res[0];
    }

    private long dfs(int cur, Map<Integer, List<Integer>> adj, int[] res, int[] vals, int k,
        Set<Integer> visited) {
        visited.add(cur);
        long sum = vals[cur];
        ...
        for (int neigh : adj.getOrDefault(cur, new ArrayList<>())) {
            if (!visited.contains(neigh)) {
                sum += dfs(neigh, adj, res, vals, k, visited);
                sum %= k; // handling overflow
            }
        }
        if (sum % k == 0) {
            res[0]++;
