...
change for an amount of money targetMoney. Both the denominations and target amount will 
be given in generic units of that currency.
...
public class ChangeInAForeignCurrency {
    ...
    // similar to CoinChangeMinCoins.java and CoinChangeCombinations.java
    public boolean canGetExactChange(int targetMoney, int[] denominations) {
        ...
        for (int i = 1; i <= targetMoney; i++) {
            for (int j = 0; j < n; j++) {
                if (i - denominations[j] >= 0 && dp[i - denominations[j]]) {
                    dp[i] = true;
                    break;
                }
            }
